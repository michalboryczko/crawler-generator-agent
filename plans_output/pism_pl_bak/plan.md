...
```css
.articles.index.content .article-preview .article-title > a[href^="/publikacje/"]
```
- **Confidence**: 0.98
...
- **category:**
  ```css
  .articles.index.content .article-preview .article-title .article-type a
  ```

- **author:**
  ```css
  .articles.index.content .article-preview .article-author a
  ```
...
**Selector:**
```css
h1.page-title, .article-view h1
```
*Main article title (listing uses h1.page-title, detail pages likely use .article-view h1)*

### 5.2. Date

**Selector:**
```css
.article-view .article-date, .article-view time
```
*Publication date (no date on listing cards)*

### 5.3. Authors

**Selector:**
```css
.article-view .article-author a, .article-author a
```
*Author names (links to /analitycy/...)*

### 5.4. Category

**Selector:**
```css
.article-view .article-type a, .article-title .article-type a
```
...
**Selector:**
```css
.article-view .article-lead, .article-view .lead, .article-view .summary
```
*Lead / summary text under the title*

### 5.6. Content

**Selector:**
```css
.article-view .article-content, .article-view .content, .article-view .text
```
...
**Selector:**
```css
.article-view .tags a, .article-view .article-tags a
```
*Tags / topics assigned to the article*

### 5.8. Breadcrumbs

**Selector:**
```css
.frontend-path a, nav.breadcrumbs a, nav[aria-label*="breadcrumb"] a
```
...
config = {
    ...
    "listing": {
        "article_link_selector": ".articles.index.content .article-preview .article-title > a[href^=\"/publikacje/\"]",
    },
    "pagination": {
        "enabled": true,
    ...
    "detail": {
        "title_selector": "h1.page-title, .article-view h1",
        "date_selector": ".article-view .article-date, .article-view time",
        "authors_selector": ".article-view .article-author a, .article-author a",
        "category_selector": ".article-view .article-type a, .article-title .article-type a",
        "lead_selector": ".article-view .article-lead, .article-view .lead, .article-view .summary",
        "content_selector": ".article-view .article-content, .article-view .content, .article-view .text",
        "tags_selector": ".article-view .tags a, .article-view .article-tags a",
        "breadcrumbs_selector": ".frontend-path a, nav.breadcrumbs a, nav[aria-label*=\"breadcrumb\"] a"
    },
    "request": {
