...
LOG_RECORD_FIELDS: dict[str, FieldDef] = {
    ...
    F.TRACE_ID: FieldDef(
        F.TRACE_ID, ESType.KEYWORD, "OTel trace ID (32 hex chars, 128-bit)", required=True
    ),
    F.SPAN_ID: FieldDef(
        F.SPAN_ID, ESType.KEYWORD, "OTel span ID (16 hex chars, 64-bit)", required=True
    ),
    F.PARENT_SPAN_ID: FieldDef(
        F.PARENT_SPAN_ID, ESType.KEYWORD, "Parent span ID (None for root, 16 hex chars)"
    ),
    ...
    F.LEVEL: FieldDef(
        F.LEVEL,
        ESType.KEYWORD,
        "Log level (DEBUG, INFO, WARNING, ERROR) - metadata only!",
        required=True,
    ),
    F.EVENT: FieldDef(
        F.EVENT, ESType.KEYWORD, "Event type (e.g., tool.input, agent.error)", required=True
    ),
    # Component info
    F.COMPONENT_TYPE: FieldDef(
        F.COMPONENT_TYPE, ESType.KEYWORD, "Type: agent, tool, llm_client", required=True
    ),
    F.COMPONENT_NAME: FieldDef(
        F.COMPONENT_NAME, ESType.KEYWORD, "Specific component name", required=True
    ),
    F.TRIGGERED_BY: FieldDef(
        F.TRIGGERED_BY, ESType.KEYWORD, "Parent component that triggered this one", required=True
    ),
    ...
    F.METRICS: FieldDef(
        ...
        "Numeric metrics",
        nested_fields={
            M.DURATION_MS: FieldDef(
                M.DURATION_MS, ESType.FLOAT, "Execution duration in milliseconds"
            ),
            M.TIME_TO_FIRST_TOKEN_MS: FieldDef(
                M.TIME_TO_FIRST_TOKEN_MS, ESType.FLOAT, "Time to first LLM token"
            ),
            ...
            M.ESTIMATED_COST_USD: FieldDef(
                M.ESTIMATED_COST_USD, ESType.FLOAT, "Estimated API cost in USD"
            ),
            M.RETRY_COUNT: FieldDef(M.RETRY_COUNT, ESType.INTEGER, "Number of retries"),
            M.CONTENT_SIZE_BYTES: FieldDef(
                M.CONTENT_SIZE_BYTES, ESType.INTEGER, "Content size in bytes"
            ),
        },
    ),
    # Tags for filtering
...
TRACE_EVENT_FIELDS: dict[str, FieldDef] = {
    ...
    F.ATTRIBUTES: FieldDef(
        ...
        nested_fields={
            ...
            D.ERROR_MESSAGE: FieldDef(
                D.ERROR_MESSAGE, ESType.TEXT, "Error message (full-text searchable)"
            ),
    ...
    if field_def.es_type == ESType.OBJECT:
        if field_def.nested_fields:
            return {
                "type": "object",
                "properties": {
                    name: field_to_es_mapping(f) for name, f in field_def.nested_fields.items()
                },
            }
        else:
            # Dynamic object
...
def generate_es_mappings(fields: dict[str, FieldDef]) -> dict:
    ...
    return {
        "properties": {name: field_to_es_mapping(field_def) for name, field_def in fields.items()}
    }
