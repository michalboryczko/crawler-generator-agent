...
class FindSelectorTool(BaseTool):
        ...
        for selector in candidates:
            ...
            if elements:
                ...
                valid = [
                    ...
                    for el in elements
                    if el.get("href")
                    and not el["href"].startswith("#")
                    and not el["href"].startswith("javascript:")
                ]
                if valid:
                    results.append({"selector": selector, "count": len(valid), "sample": valid[:3]})
    ...
    def _find_pagination_selectors(self, hint: str | None) -> dict[str, Any]:
        ...
        for selector in candidates:
            ...
            if elements:
                results.append(
                    {"selector": selector, "count": len(elements), "sample": elements[:3]}
                )
